...
class DeepResNet(ph.Widget):
    ...
    def _build(self):
        self._input_layer = ph.Linear('input_layer', self._input_size, self._hidden_size)

        res_layers = self._res_layers = list()
        for i in range(self._num_layers):
            res_layer = ph.ResidualLinear(f'res_{str(i)}', self._hidden_size)
            res_layers.append(res_layer)

        self._output_layer = ph.Linear('output_layer', self._hidden_size, self._output_size)
